...
void drawNumHits(){
  ...
  */
  ...
  for(int ij=0; ij<nFiles; ++ij){
    inF[ij] = TFile::Open(("../../test/timingStudies/"+optionType+"/"+"JOB_"+nameFiles.at(ij)+"_3fC/"+"/OutTimeHGC_RecHits_"+nameFiles.at(ij)+"_3fC.root").c_str());
  }
  ...
  for(int iF=0; iF<nFiles; ++iF){
	...
	hDummySt[iR]->SetLineColor(iColors[iR+2]);
      ...
      energy[iF][iR] = (TH1F*)(inF[iF]->Get(Form("ana/hEnergyWithTime_Eta_dRadius_Eta1.65-1.85_dRadius%d", iR)));
	...
	numHitsWithTime[iF][iR]->SetBinContent(ij, numEvt);
      ...
      energy[iF][iR]->SetLineColor(iColors[iR+2]);
      energy[iF][iR]->SetMarkerColor(iColors[iR+2]);
      energy[iF][iR]->SetLineWidth(2);
      energy[iF][iR]->SetMarkerStyle(iStyle[iR]);
    }
  }
  ...
  for(int iF=0; iF<nFiles; ++iF){
    ...
    ch_Energy[iF]->cd();
    energy[iF][0]->GetXaxis()->SetTitle("#Sigma recHit energy (1.65 < |#eta| < 1.85)");
    energy[iF][0]->GetXaxis()->SetRangeUser(0., ptValues.at(iF) + 10.);
    energy[iF][0]->GetYaxis()->SetTitle(" events");
    //    numHitsWithTimeDistr[iF][0]->GetXaxis()->SetRangeUser(0., 20.);
    //    numHitsWithTimeDistr[iF][0]->GetYaxis()->SetRangeUser(0., 1.01);
    energy[iF][0]->Draw("");

    for(int iR=1; iR<nBinsRad; ++iR){
      energy[iF][iR]->Draw("same");
    }
    tL.DrawLatex(0.7,0.6,nameFiles.at(iF).c_str());
    //    legTGM->Draw("same");
    legTGM2->Draw("same");
    ch_Energy[iF]->Print((folder+"/h_recEnergy_file"+nameFiles.at(iF)+".png").c_str(), "png");
    ch_Energy[iF]->Print((folder+"/h_recEnergy_file"+nameFiles.at(iF)+".pdf").c_str(), "pdf");
    ch_Energy[iF]->Print((folder+"/h_recEnergy_file"+nameFiles.at(iF)+".root").c_str(), "root");
  }
