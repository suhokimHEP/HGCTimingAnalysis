...
UtilClasses::UtilClasses(float genEta, float genPhi):_genEta(genEta), _genPhi(genPhi)
{}
...
*/
 float UtilClasses::deltaEta(float eta1, float eta2){
    float dEta = eta1 - eta2;
    return dEta;
}

 float UtilClasses::deltaR(float eta1, float eta2, float phi1, float phi2) {
    float dEta = deltaEta(eta1, eta2);
    float dPhi = reco::deltaPhi(phi1, phi2);
    return sqrt( dEta*dEta + dPhi*dPhi );
}

 float UtilClasses::deltaX(float x1, float x2, float y1, float y2) {
    float dX = x1 - x2;
    float dY = y1 - y2;
...
// need correct sign for z
 float UtilClasses::etaPhiZtoX( float eta, float phi, float z )
{
  float t = exp( -1 * eta );
...
// need correct sign for z
 float UtilClasses::etaPhiZtoY( float eta, float phi, float z )
{
  float t = exp( -1 * eta );
...
// go from HGC layer to z in cm
 float UtilClasses::layerToZ( int layer, float eta ) 
{
    map<int, float> lToZ;
    lToZ[0]  = 320.75;
    lToZ[1]  = 321.50;
...
int UtilClasses::Ztolayer(float z, float eta ) 
{
    map<int, float> lToZ;
    lToZ[0]  = 320.75;
    lToZ[1]  = 321.50;
 ...
 float UtilClasses::dsGenRecHit(float genEta, float genPhi, int recHitLayer, float recHitX, float recHitY )
{
 ...
 float UtilClasses::dsGenRecoObj(float genEta, float genPhi, float recoObjZ, float recoObjX, float recoObjY )
{
...
void UtilClasses::layerIntersection(std::array<double,3> &to,
				      ...
				      ) const
{
 ...
 vector<TH1*> UtilClasses::initHists( string name )
{
  vector<TH1*> v;
 ...
 void UtilClasses::writeHists( vector<TH1*> hists )
{
  for( uint i=0; i<hists.size(); i++ ) hists[i]->Write();
}

 void UtilClasses::drawAndSaveHists( vector<TH1*> hists, TCanvas *c, bool doLogs)
{
