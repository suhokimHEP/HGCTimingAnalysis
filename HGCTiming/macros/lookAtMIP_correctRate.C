...
void lookAtMIP_correctRate(){
 ...
 /*da 41 a 50*/	       87.582, 87.582, 87.582, 87.582, 87.582, 87.2146, 86.8883, 86.9295, 86.9295, 86.9295}; 
  ...
  TH1F* cm2_vsEta_All[14];
   ...
   //for(int ij=0; ij<2000; ++ij){
	 ...
	 for(auto iR : rhX[iL]){
	   ...
	   if(trkEta[iL][iTc] * rhZ[iL][iRc] < 0.) continue;
	   float dX = iR - iT;
	   ...
	   std::pair<int, std::pair<int, int> > channel = std::pair<int, std::pair<int, int> >(iL, coordPair); 	  
	   ...
	   enePerRh[channel].push_back(rhEne[iL][iRc]);
	   mipPerRh[channel].push_back(rhMip[iL][iRc]);
	   ...
	   enePerPhi[channelPhi].push_back(rhEne[iL][iRc]);
	   mipPerPhi[channelPhi].push_back(rhMip[iL][iRc]);
	   mipAll->Fill(rhMip[iL][iRc]);
	   // std::cout << " rhMip[iL][iR] = " << rhMip[iL][iR] << std::endl;
   ...
   for(int ij=0; ij<14; ++ij){
     ...
     ratePercm2_vsEta[ij]->GetYaxis()->SetRangeUser(0., 50.);
     ...
     ratePercm2_vsEta_All[ij]->GetYaxis()->SetRangeUser(0., 50.);
  ...
  tcC_Phi->Print("plotsMinBias_correctRate/cellsPhi/h2_iRvsLayer_MIP.png", "png");
  tcC_Phi->Print("plotsMinBias_correctRate/cellsPhi/h2_iRvsLayer_MIP.root", "root");
